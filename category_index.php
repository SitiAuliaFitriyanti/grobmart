...
while ($fetch_category = mysqli_fetch_assoc($query_category)) {
    ?>
                      <tr>
                        ...
                        <td>
                          <?php if ($fetch_category['category_ticket_id'] != 0) {?>
                            <a class="btn btn-warning btn-sm" href="category_update.php?id=<?=$fetch_category['category_ticket_id']?>"><i class="fa fa-edit"></i>edit</a>
                            <a class="btn btn-danger btn-sm" href="category_delete.php?id=<?=$fetch_category['category_ticket_id']?>" onclick="return confirm('Are you sure you want to delete this data?')"><i class="fa fa-trash"></i> delete</a>
                          <?php }?>
                        </td>
                      </tr>
                    <?php
